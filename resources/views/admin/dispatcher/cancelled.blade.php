<div class="dispatcher-section-head">
    <h2 class="dispatcher-section-title">{{ __('messages.cancelled_list') }}</h2>
    <div style="display: flex; align-items: center; gap: 0.75rem;">
        @if(!empty($orders) && count($orders) > 0)
            <span class="dispatcher-section-meta">{{ $orders->total() }} {{ Str::plural('order', $orders->total()) }}</span>
        @endif
        <div class="export-menu">
            ...
            <div class="export-dropdown">
                <a href="{{ route('dispatcher.export.csv', ['type' => 'cancel']) }}" class="export-item">
                    <i class="fas fa-file-csv"></i>
                    Download CSV
                </a>
                <a href="{{ route('dispatcher.export.pdf', ['type' => 'cancel']) }}" target="_blank" class="export-item">
...
@if(!empty($orders) && count($orders) > 0)
                ...
                <div class="order-card-body">
                    ...
                    <div class="order-head">
                        <div class="order-identity">
                            <div class="order-avatar" style="background: linear-gradient(135deg, #ef4444, #b91c1c);">
                            ...
                            <div style="min-width: 0;">
                                ...
                                <div class="order-tags">
                                    ...
                                    <span class="ord-badge ord-badge-cancel">
                                        <i class="fas fa-times"></i>
                                        {{ __('messages.cancelled') }}
                                    </span>
                                    @if(!empty($order->payment) && !empty($order->payment->gateway))
                                        <span class="ord-badge ord-badge-gateway">
    ...
    <input type="hidden" id="tab_type" value="cancel">
    <div class="custom_pagination">
        {{ $orders->links() }}
    </div>
@else
    <div class="dispatcher-empty">
        <div class="dispatcher-empty-icon rose">
            <i class="fas fa-ban"></i>
        </div>
        <h3 class="dispatcher-empty-title">No cancelled orders</h3>
        <p class="dispatcher-empty-text">Cancelled orders will be listed here.</p>
    </div>
@endif
