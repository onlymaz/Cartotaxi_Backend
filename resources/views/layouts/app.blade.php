...
<head>
    ...
    <script>
        function showLoader() { $('#bodyContent .loader-layout').show(); }
        function hideLoader() { $('#bodyContent .loader-layout').hide(); }
    </script>
...
<body>
    <div class="ct-app">
        <div id="ct-sidebar-backdrop"></div>
        @auth
            <!-- Sidebar -->
            ...
            <div class="ct-main">
                ...
                <header class="ct-header">
                    ...
                    <div class="ct-header-right">
                        ...
                        <div class="ct-dropdown">
                            <button class="ct-header-btn" id="notifBtn" onclick="loadNotifications(); this.parentElement.classList.toggle('open')">
                                <i class="fas fa-bell"></i>
                                <span id="notifBadge" class="ct-badge" style="display:none; position:absolute; top:2px; right:2px; min-width:18px; height:18px; padding:0 4px; background:#ef4444; color:white; font-size:0.625rem; font-weight:600; border-radius:9999px; align-items:center; justify-content:center; border:2px solid white;"></span>
                            </button>
    ...
    <script>
        ...
        document.getElementById('menuToggle')?.addEventListener('click', () => {
            ...
            document.getElementById('ct-sidebar-backdrop').classList.toggle('active');
        });
        // Close sidebar when backdrop clicked
        document.getElementById('ct-sidebar-backdrop')?.addEventListener('click', () => {
            document.getElementById('sidebar').classList.remove('open');
            document.getElementById('ct-sidebar-backdrop').classList.remove('active');
        });
            ...
            $.get("{{ url('notifications') }}", function(data) {
                if (data.data && data.data.length > 0) {
                    ...
                    data.data.forEach(function(n) {
                        html += '<div style="padding: 0.75rem; border-bottom: 1px solid #f1f5f9;">' +
                            '<div style="font-size: 0.875rem;">' + n.notifications_text + '</div>' +
                            '<div style="font-size: 0.75rem; color: #94a3b8; margin-top: 0.25rem;">' + n.created_at + '</div>' +
                        '</div>';
                    });
                    $('#notifList').html(html);
                    // Update badge count (additive)
        ...
        // Auto-load badge on page load (silent, won't affect UI if it fails)
        $(function() {
            $.get("{{ url('notifications') }}", function(data) {
                if (data.data && data.data.length > 0) {
                    $('#notifBadge').text(data.data.length > 99 ? '99+' : data.data.length).show();
                }
            });
        });
        ...
        $(document).on('click', '.Reload', function() {
            var table = $('body').find('table').attr('id');
    ...
    <style>
        ...
        .card-header { padding: 1rem 1.5rem; border-bottom: 1px solid #e2e8f0; background: #f8fafc; border-radius: 12px 12px 0 0; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; }
        ...
        .btn { display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 0.5rem 1rem; font-size: 0.875rem; font-weight: 500; border-radius: 0.5rem; cursor: pointer; transition: all 0.2s; border: 1px solid transparent; text-decoration: none; }
        .btn-success { background: #84cc16; border-color: #84cc16; color: #1e293b; }
        .btn-success:hover { background: #65d419; border-color: #65d419; }
        .btn-primary { background: #84cc16; border-color: #84cc16; color: #1e293b; }
        .btn-primary:hover { background: #65d419; border-color: #65d419; }
        .btn-danger { background: #ef4444; border-color: #ef4444; color: white; }
        .btn-danger:hover { background: #dc2626; border-color: #dc2626; }
        .btn-secondary { background: white; border-color: #e2e8f0; color: #475569; }
        ...
        @media (max-width: 1024px) {
            #ct-sidebar-backdrop.active { display: block; }
        }
    </style>
